...
do  
    ...
    result=`cat telnet_domain_result.txt | grep -B 1 \] | grep [0-9] | awk '{print $3}' | cut -d '.' -f 1,2,3,4`
    echo "$result"
    if [ -n "$result" ]; then
        echo "$domain|$port|" >> telnet_node_pre-checked.txt
		
    else
	
		newport=`expr $port + 1`
        	echo $(date +"%y-%m-%d %H:%M:%S") "$domain:$port Fail! " >> telnet_domain_failed.txt
		echo "$domain|$newport|" >> telnet_node_pre-checked.txt
	...
	mv  telnet_node_pre-checked.txt  nodelist.txt
